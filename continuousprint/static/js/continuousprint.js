					...
					if (r.queue.length > 0) {
						...
						for(var i = 0; i < r.queue.length; i++) {
							...
							if (file["time"] == undefined) {
								...
								row = $("<div style='padding: 10px;border-bottom: 1px solid #000;"+(i==0 ? "background: #f9f4c0;" : "")+"'><div style='width:100%;display: flex; flex-direction: row; justify-content: flex-start; align-items: stretch;'><input class='fa fa-text' type = 'text' style=' flex-grow: "+self.textGrow[i]+";flex-shrink:1; width:10px; max-width:60px; max-height:8px;' data-index='"+i+"' value='" + file.count.toString() + "'/><p style='flex-shrink: 10; overflow-x:scroll;max-width:70%'> " + file.name + "</p><div style='justify-self:end'" + other + "<i style='cursor: pointer' class='fa fa-minus text-error' data-index='"+i+"'></i></div></div></div>");
