					...
					if (r.queue.length > 0) {
						...
						for(var i = 0; i < r.queue.length; i++) {
							...
							if (file["time"] == undefined) {
								...
								row = $("<div style='padding: 10px;border-bottom: 1px solid #000;"+(i==0 ? "background: #f9f4c0;" : "")+"'><input class='fa fa-text' type = 'text' style='width:30px; max-width:30px; max-height:8px; height 8px;' data-index='"+i+"' value='" + file.count.toString() + "'/><p style='flex-shrink: 10; overflow-x:scroll;max-width:" + nameWidth + "%'> " + file.name + "</p><div style='display: table-cell;' class= 'pull-right'>" + other + "<i style='cursor: pointer' class='fa fa-minus text-error' data-index='"+i+"'></i></div></div>");
