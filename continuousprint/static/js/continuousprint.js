					...
					if (r.queue.length > 0) {
						...
						for(var i = 0; i < r.queue.length; i++) {
							...
							if (file["time"] == undefined) {
								...
								row = $("<div style='padding: 10px;border-bottom: 1px solid #000;"+(i==0 ? "background: #f9f4c0;" : "")+"'>" + file.name + "<div class='pull-right'>" + other + "<i style='cursor: pointer' class='fa fa-minus text-error' data-index='"+i+"'></i></div>" + "<input class='pull-left' type = 'text' data-index='"+i+"' value='"+file.count.toString()+"'/>"+"</div>");
