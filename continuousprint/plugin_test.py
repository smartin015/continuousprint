...
class TestStartup(unittest.TestCase):
    ...
    def testObicoFound(self):
        ...
        self.assertEqual(p._get_key(Keys.NETWORK_QUEUES), False)  # PeerPrint
        self.assertEqual(p._get_key(Keys.MATERIAL_SELECTION), False)  # Spoolmanager
        self.assertEqual(p._get_key(Keys.RESTART_ON_PAUSE), True)  # Obico

    def testSpoolManagerFound(self):
        p = setupPlugin()
        p._plugin_manager.plugins.get.side_effect = (MagicMock(), None)

        p._setup_thirdparty_plugin_integration()

        p._plugin_manager.plugins.get.assert_any_call("SpoolManager")
        self.assertEqual(p._get_key(Keys.NETWORK_QUEUES), False)  # PeerPrint
        self.assertEqual(p._get_key(Keys.MATERIAL_SELECTION), True)  # Spoolmanager
        self.assertEqual(p._get_key(Keys.RESTART_ON_PAUSE), False)  # Obico

    def testPeerPrintFound(self):
        p = setupPlugin()
        p._plugin_manager.plugins.get.side_effect = (None, MagicMock())

        p._setup_thirdparty_plugin_integration()

        p._plugin_manager.plugins.get.assert_any_call("octoprint_peerprint")
        self.assertEqual(p._get_key(Keys.NETWORK_QUEUES), True)  # PeerPrint
        self.assertEqual(p._get_key(Keys.RESTART_ON_PAUSE), False)  # Obico
        self.assertEqual(p._get_key(Keys.MATERIAL_SELECTION), False)  # SpoolManager
    ...
    def testQueues(self):
        ...
        p._set_key(Keys.NETWORK_QUEUES, True)
        p._peerprint = MagicMock()
        ...
        p._init_queues(netcls=MagicMock(), localcls=MagicMock())
