...
class LocalQueue(AbstractEditableQueue):
    def __init__(self, queries, queueName: str, strategy: Strategy, profile: dict, path_on_disk_fn):
        super().__init__()
    ...
    def add_job(self) -> JobView:
        return queries.newEmptyJob(self.ns)

    def add_set(self, job_id, data) -> SetView:
        return queries.appendSet(self.ns, job_id, data)

    def mv_set(self, set_id, after_id, dest_job):
        return queries.moveSet(set_id, after_id, dest_job)

    def mv_job(self, job_id, after_id):
        return queries.moveJob(job_id, after_id)

    def edit_job(self, job_id, data):
        return queries.updateJob(job_id, data)

    def rm_multi(self, job_ids, set_ids) -> dict:
        return queries.remove(job_ids, set_ids)

    def import_job(self, gjob_path: str) -> dict:
        out_dir = str(Path(gjob_path).stem)
        os.makedirs(dirname, exist_ok=True)
        manifest, filepaths = unpack_job(self._path_on_disk(gjob_path), self._path_on_disk(dirname))
        return queries.importJob(self.ns, manifest, out_dir)

    def export_job(self, job_id: int, dest_dir: str) -> str:
        j = queries.getJob(job_id)
        filepaths = dict([
                (s.path, self._path_on_disk(s.path))
                for s in j.sets
            ])
        with tempfile.NamedTemporaryFile(suffix=".gjob", dir=dest_dir, delete=False) as tf:
    ...
    def reset_jobs(self, job_ids):
        return queries.resetJobs(job_ids)
