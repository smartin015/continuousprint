...
class CPQPlugin(ContinuousPrintAPI):
    ...
    def _setup_thirdparty_plugin_integration(self):
        ...
        # PeerPrint is only needed for network printing
        ppplugin = self._plugin_manager.plugins.get("octoprint_peerprint")
        if ppplugin is not None and ppplugin.enabled:
            self._peerprint = ppplugin.implementation
            self._logger.info("PeerPrint found - enabling network queues")
            self._set_key(Keys.NETWORK_QUEUES, True)
        else:
            self._peerprint = None
            self._logger.info(
                "No peerprint instance found; network queues will be disabled"
            )
            self._set_key(Keys.NETWORK_QUEUES, False)
    ...
    def _init_queue(self, q, netcls=NetworkQueue, localcls=LocalQueue):
        if q.name == ARCHIVE_QUEUE:
            return
        elif q.name == DEFAULT_QUEUE:
            self.q.add(
                q.name,
                localcls(
                    self._queries,
                    q.name,
                    Strategy.IN_ORDER,  # TODO set strategy from q
                    self._printer_profile,
                    self._path_on_disk,
                    self._add_folder,
                ),
            )
        elif self._get_key(Keys.NETWORK_QUEUES, False):  # Peerprint queue
            nq = netcls(
                q.name,
                self._peerprint,
                self._logger.getChild(q.name),
                Strategy.IN_ORDER,  # TODO set strategy from q
            ...
            nq.connect()
            self.q.add(q.name, nq)

    def _init_queues(self, netcls=NetworkQueue, localcls=LocalQueue):
        ...
        for q in self._queries.getQueues():
            try:
                self._init_queue(q, netcls, localcls)
    ...
    def cleanup_fileshare(self):
        ...
        return self._peerprint.get_plugin().cleanup_fileshare(keep_hashes)
    ...
    def _commit_queues(self, added, removed, netcls=NetworkQueue, localcls=LocalQueue):
        for name in removed:
            self.q.remove(name)
        for a in added:
            q = Queue(
