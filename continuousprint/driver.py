...
class ContinuousPrintDriver:
    ...
    def _state_spaghetti_recovery(self, a: Action, p: Printer):
      self._set_status(f"Cancelling print (spaghetti seen early in print)")
      if p == Printer.PAUSED:
    ...
    def _state_success(self, a: Action, p: Printer):
      ...
      if idx is not None:
          ...
          if self._intent == path and self._cur_path == path:
              ...
              self._logger.info(f"Current queue item {path} not matching intent {self._intent}, current path {self._cur_path} - no completion")
      self.retries = 0
