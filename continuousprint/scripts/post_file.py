...
class Runner:
    def __init__(self, args):
        ...
        self.manifest = dict(
            ...
            sets=[dict(path=fname, count=1)],
        )
        print(f"Posting manifest: {self.manifest}")
        self.hash_ = self.fs.post(
            self.manifest, {fname: str(Path(self.td.name) / fname)}
        )

        print(f"Connecting to '{args.ns}', addr {args.addr}")
        self.t = threading.Thread(target=self.sync_status, daemon=True)
        self.lpq = LANPrintQueue(
            args.ns, args.addr, self.ready, logging.getLogger(args.ns)
        )
        self.started = False
        ...
        while True:
            ...
            self.lpq.q.syncPeer(
                dict(
                    ...
                    status="SCRIPT",
                    run=None,
                    fs_addr=f"{self.fs.host}:{self.fs.port}",
                )
            )
            if not sub:
